(function (Commands) {
  ...
  Commands[Commands['CMPP_QUERY'] = 0x00000006] = 'CMPP_QUERY'
  Commands[Commands['CMPP_QUERY_RESP'] = 0x80000006] = 'CMPP_QUERY_RESP'

  Commands[Commands['CMPP_CANCEL'] = 0x00000007] = 'CMPP_CANCEL'
  Commands[Commands['CMPP_CANCEL_RESP'] = 0x80000007] = 'CMPP_CANCEL_RESP'
...
exports.CommandsDescription = {
  ...
  // CMPP_TERMINATE, // 无消息实体
  // CMPP_TERMINATE_RESP, // 无消息实体
  CMPP_SUBMIT: [
      ...
      { name: 'Msg_Length', type: 'number', length: 1 } // Msg_Fmt 值为 0 时：< 160 个字节；其它 <= 140 个字节
      ,
  ...
  CMPP_QUERY: [
    { name: 'Time', type: 'string', length: 8 },
    { name: 'Query_Type', type: 'number', length: 8 },
    { name: 'Query_Code', type: 'string', length: 10 },
    { name: 'Reserve', type: 'string', length: 8 },
  ],
  CMPP_QUERY_RESP: [
    { name: 'Time', type: 'string', length: 8 },
    { name: 'Query_Type', type: 'number', length: 1 },
    { name: 'Query_Code', type: 'string', length: 10 },
    { name: 'MT_TLMsg', type: 'number', length: 4 },
    { name: 'MT_Tlusr', type: 'number', length: 4 },
    { name: 'MT_Scs', type: 'number', length: 4 },
    { name: 'MT_WT', type: 'number', length: 4 },
    { name: 'MT_FL', type: 'number', length: 4 },
    { name: 'MO_Scs', type: 'number', length: 4 },
    { name: 'MO_WT', type: 'number', length: 4 },
    { name: 'MO_FL', type: 'number', length: 4 },
  ],
  CMPP_DELIVER: [
      { name: 'Msg_Id', type: 'buffer', length: 8 },
  ...
  // 当 ISMG 向 SP 送交状态报告时，信息内容字段（Msg_Content）格式定义如下
  ...
  CMPP_CANCEL_RESP: [
    { name: 'Success_Id', type: 'number', length: 1 },
  ],
  // CMPP_ACTIVE_TEST, // 无消息实体
  CMPP_ACTIVE_TEST_RESP: [
    { name: 'Reserved', type: 'string', length: 1 }, // 保留
  ]
}
