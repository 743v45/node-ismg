...
class ISMG extends EventEmitter {
  ...
  setAuthentication(authentication) {
    // 鉴权模块, 默认不鉴权
    this.authentication = authentication || function ({Source_Addr, AuthenticatorSource, Timestamp, remoteAddress} = {}) {
      return {
        Status: 0,
        AuthenicatorISMG: '',
      };
  ...
  listen(port, callback) {
    ...
    this._server.listen(this.getPort(), '0.0.0.0', function(err) {
      if (err) {
