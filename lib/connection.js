...
class Connection extends EventEmitter {
  ...
  async handleConnect(header, body) {
    ...
    if (Version > _this.MaxVersion) {
      ...
      return connectResp(header.Sequence_Id, result);
    }
    ...
    const auth = await _this.ismg.authentication({
      ...
      AuthenticatorSource,
      Timestamp,
      remoteAddress: _this.socket.remoteAddress, // 用于鉴定 IP 有效性
    });
    ...
    function connectResp(Sequence_Id, result) {
      return _this.sendResponse(cmdCfg.Commands.CMPP_CONNECT_RESP, Sequence_Id, result).finally(() => {
        if (result.Status !== 0) {
          // todo: add log
          _this.disconnect();
        } else {
